...
const TrustBar = () => (
  <section className="border-b border-border bg-white">
    <div className="max-w-[1280px] mx-auto px-5 md:px-8">
      <div className="flex items-center justify-center md:justify-between flex-wrap gap-0 divide-x divide-border">
        {trustItems.map((item) => (
          <div key={item.label} className="flex items-center gap-2.5 px-6 py-4 flex-1 justify-center min-w-[160px]">
            <span className="text-primary shrink-0">{item.icon}</span>
            <span className="font-jost text-[13px] font-medium text-foreground whitespace-nowrap">{item.label}</span>
          </div>
        ))}
      </div>
    </div>
